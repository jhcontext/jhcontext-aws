...
Two separate subpackages, one per education pipeline variant. The modules
do not share pipeline code: each mirrors its own architectural description
verbatim so that a reviewer can map flows back to the scenario descriptions
...
| `rubric_feedback_grading.py` | **Rubric-grounded grading** — auditable AI assessment | 3-agent chain: `ingestion → criterion-scoring → feedback` (per-sentence envelope emission at the feedback step), plus isolated equity workflow, TA-review flow with temporal oversight, and a combined audit that runs three verifiers | Three scenarios — (A) identity-blind grading, (B) rubric-grounded LLM feedback, (C) human–AI collaborative grading |
