...
class RubricGradingFlow(Flow, ContextMixin):
    ...
    def feedback_generation(self, scoring_output):
        ...
        # Record the aggregate feedback step
        self._persist_step(
            step_name="feedback",
        ...
        # Emit one envelope + PROV entity per feedback sentence (Scenario B)
        ...
        for i, fs in enumerate(sentences, start=1):
            ...
            self._persist_step(
                step_name=f"feedback-{fs_id}",
                agent_id="did:university:rubric-feedback-agent",
                output=json.dumps(fs, ensure_ascii=False),
                artifact_type=ArtifactType.SEMANTIC_EXTRACTION,
                started_at=ts,
                ended_at=ts,
                used_artifacts=["art-ingestion"],
            )
            # Attach rubric-binding attributes to the PROV entity so
            # ``verify_rubric_grounding`` can audit them later.
            ...
            prov.set_entity_attribute(
                art_id, "promptTemplateHash", fs.get("prompt_template_hash", PROMPT_TEMPLATE_HASH)
            )
            if ev_hash:
            ...
            # Link every feedback-sentence entity back to the submission
